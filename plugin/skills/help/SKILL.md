...
- **Skill name provided** (e.g. `plan`, `spec`) → print the focused cheat sheet for that skill only.
...
**No match:** Print:
> "`<token>` is not a recognized FSD skill. Available skills: new-project, roadmap, spec, spec-update, plan, plan-update, execute-plan, restructure. Run `/fsd:help` with no args for the full overview."
